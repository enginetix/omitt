...
win <- 100
Wd <- data.frame(Date = ts[i:(i+win)], Open = Opdata[i:(i+win)])


# model the clustered data
subset <- 20
M <- win/subset #nrow(Opdata)/10 # use only 1/10 of the rows for BIC
...
par(mfrow = c(2,2))
plot.Mclust(mdl1, what = "density")
plot.Mclust(mdl1, what = "classification")
plot.Mclust(mdl2, what = "density")
plot.Mclust(mdl2, what = "classification")

print(mdl2$parameters$mean)
